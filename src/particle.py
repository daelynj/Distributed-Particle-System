...
def smoke_machine():
  ...
  color_probabilities = ([0] * 32) + ([1] * 32) + ([2] * 32) + ([3] * 2) + ([4] * 2) 

  def create():
    for _ in range(random.choice([0,0,0,0,0,0,0,1,2,3])):
      c = colors[random.choice(color_probabilities)]
      s = random.randint(4, 8) if c in ['yellow', 'orange'] else random.randint(10, 15)
      behaviour = ascending(1), kill_at(1000, 1000), fan_out(400), wind(1, 15), age(1), grow(0.5)
